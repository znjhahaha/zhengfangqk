...
export async function getCourseSelectionDetails(
  ...
  try {
    ...
    console.log(`  - 课程数据中的参数: _rwlx=${courseData._rwlx}, _xklc=${courseData._xklc}, _xkly=${courseData._xkly}, _xkkz_id=${courseData._xkkz_id}`)
    ...
    console.log(`  - 完整courseData对象:`, JSON.stringify({ _rwlx: courseData._rwlx, _xklc: courseData._xklc, _xkly: courseData._xkly, _xkkz_id: courseData._xkkz_id }, null, 2))
    ...
    // 如果所有来源都没有（undefined或null），则根据kklxdm计算默认值
    if (rwlx === null || rwlx === undefined) {
      ...
      const source = courseData._rwlx ? '课程数据' : (hiddenParams.rwlx ? '页面隐藏参数' : '选课参数')
      console.log(`✅ 使用${source}的rwlx=${rwlx}`)
    }
    
    if (xklc === null || xklc === undefined) {
      ...
      const source = courseData._xklc ? '课程数据' : (hiddenParams.xklc ? '页面隐藏参数' : '选课参数')
      console.log(`✅ 使用${source}的xklc=${xklc}`)
    }
    
    if (xkly === null || xkly === undefined) {
      xkly = '0'
    } else {
      const source = courseData._xkly ? '课程数据' : (hiddenParams.xkly ? '页面隐藏参数' : '选课参数')
      console.log(`✅ 使用${source}的xkly=${xkly}`)
    }
